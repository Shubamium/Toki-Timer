...
export default function Timezone() {
  ...
  function handleAddLocation(place) {
    const hasBeenAddedBefore = savedLocation.find((val) =>
      _.isEqual(val.coord, place.coord)
    );
    // If there is no place in the list that has the same exact coordinates
    if (!hasBeenAddedBefore) {
      setSavedLocation((prev) => {
        const mutatedData = [...prev, place];
        localStorage.setItem("saved-location", JSON.stringify(mutatedData));
        return mutatedData;
      });
  ...
  async function searchLocalPosition(lat, long) {
    ...
    if (queryRes.status === 200) {
      ...
      //Show Error
      setLocalLocation({
        message: "Couldn't find the local location from the timezone database!",
      });
    }
  }
  ...
  return (
    <StyledTimezone>
      ...
      <div className="tz-content">
        {currentLocation && (
          <StyledLocalLocation>
            <div className="local-current">
              ...
              {localLocation.name ? (
                <Location
                  className={"localLocation"}
                  withDate={true}
                  place={localLocation}
                />
              ) : (
                <p>{localLocation.message}</p>
              )}
            </div>
        ...
        <div>
          <LocationList
            listOnClick={handleRemoveLocation}
            list={savedLocation}
          />
        </div>
      </div>
...
function getLongitudeTime(long) {
  ...
  const stringRes = `${Math.floor(hourDelay)}:${Math.floor(
    minuteDelay
  )}:${Math.floor(secondDelay)}`;
